...
def identify_sencell(sen_gene_ls,gene_cell,edge_index_selfloop,attention_scores):
    ...
    cell_index=gene_cell.shape[0]
    res=[]
    while cell_index<gene_cell.shape[0]+gene_cell.shape[1]:
    ...
    print("the number of identified sen cells:", res1[res1>threshold].shape)
    
    predicted_cell_indexs=torch.arange(gene_cell.shape[0],gene_cell.shape[0]+gene_cell.shape[1])[res1>threshold]
    return predicted_cell_indexs


def identify_sengene(sencell_dict,gene_cell,edge_index_selfloop,attention_scores):
    ...
    while gene_index<gene_cell.shape[0]:
        connected_cells=edge_index_selfloop[0][edge_index_selfloop[1] == gene_index].to('cpu')
    ...
    predicted_gene_indexs=torch.arange(gene_cell.shape[0])[res1>threshold]
    return predicted_gene_indexs
...
def identify_sengene_v2(new_data,
    ...
    # create SnC column
    row_indices= np.array(list(sencell_dict.keys()))-new_data.shape[1]
    ...
    for cell_type in cell_types:
        ...
        if 'SnC' in adata_deg_sub.obs['is_sen'].values:
            ...
            filtered_degs = degs[degs['p_val_adj'] < 0.05]
            if len(filtered_degs)>0:
                print(f'find {len(filtered_degs)} degs for {cell_type}')
                gene_names=filtered_degs['gene'].tolist()
                new_sen_gene_ls=[all_genes_names.index(i) for i in gene_names if i in all_genes_names]
                results_genes=results_genes+new_sen_gene_ls[:5]
            else:
                print(f'no deg genes in {cell_type}')
                
    results_genes=torch.tensor(list(set(results_genes)))
    num=len(results_genes)
    print(f'Get {num} degs!')
    sorted_sengene_ls=get_sorted_sengene(sencell_dict,gene_cell,edge_index_selfloop,attention_scores,sen_gene_ls)
    new_sengene_ls=torch.cat([sorted_sengene_ls[num:],results_genes])
    
    return new_sengene_ls
    
    
def identify_sencell_v1(sen_gene_ls,gene_cell,edge_index_selfloop,attention_scores,graph_nx,celltype_names):
    gene_index=torch.tensor(sen_gene_ls)
    ...
    while cell_index<gene_cell.shape[0]+gene_cell.shape[1]:
        ...
        if len(connected_genes[gene_mask[connected_genes]])==0:
            ...
            if celltype_names[cluster] == 'Fibrotic fibroblast':
                sub_scores.append(attention_s)
            
        cell_index+=1

    res1=torch.tensor(res)
    sub_scores=torch.tensor(sub_scores)
    distance=torch.max(res1)-torch.max(sub_scores)
    
    for cell_index in range(gene_cell.shape[0],gene_cell.shape[0]+gene_cell.shape[1]):
        cluster=graph_nx.nodes[int(cell_index)]['cluster']
        if celltype_names[cluster] == 'Fibrotic fibroblast':
            res1[cell_index-gene_cell.shape[0]]=res1[cell_index-gene_cell.shape[0]]+1*distance
    ...
    #     predicted_cell_indexs=torch.arange(gene_cell.shape[0],gene_cell.shape[0]+gene_cell.shape[1])[res1>threshold]
    # top 600
    predicted_cell_indexs=torch.argsort(res1)[-600:]+gene_cell.shape[0]
    
    return predicted_cell_indexs,res1
...
def identify_sencell_v3(sen_gene_ls,gene_cell,edge_index_selfloop,attention_scores,graph_nx,celltype_names):
    print('identify_sencell_v3 ...')
    # attention_scores=attention_scores.to('cpu')
    # edge_index_selfloop=edge_index_selfloop.to('cpu')
    ...
    for cell_index in range(gene_cell.shape[0],gene_cell.shape[0]+gene_cell.shape[1]):
        ...
        if len(connected_genes[gene_mask[connected_genes]])==0:
            ...
            if cluster in ct_specific_scores:
                ct_specific_scores[cluster].append(attention_s)
                ct_specific_cell_indexs[cluster].append(cell_index)
            else:
                ct_specific_scores[cluster]=[attention_s]
                ct_specific_cell_indexs[cluster]=[cell_index]
            

    res1=torch.tensor(res)

    # Convert the dictionary to a format suitable for seaborn's boxplot function
    data_for_plotting = []
    categories = []
    ...
    for key, values in ct_specific_scores.items():
        values_ls=torch.tensor(values).tolist()
        data_for_plotting.extend(values_ls)
        categories.extend([celltype_names[key]] * len(values))
        
        # 统计离群点个数以及生成离群点的index
        counts,snc_index=calculate_outliers(values_ls)
        if counts>=10:
            snc_index=[ct_specific_cell_indexs[key][i] for i in snc_index]
            snc_indexs=snc_indexs+snc_index
        # print(celltype_names[key],counts,end=", ")
    
    
    return snc_indexs, res1
...
def extract_cell_indexs(ct_specific_scores):
    ...
    # generate boxplot for each cell type
    # Create a DataFrame suitable for Seaborn
    # import pandas as pd
    # df = pd.DataFrame({
    #     "Cell Type": categories,
    #     "Scores": data_for_plotting
    # })

    # # Create the boxplot
    # plt.figure(figsize=(15,10))
    # sns.boxplot(x='Scores', y='Cell Type', data=df,orient='h')
    # plt.title('Box Plot of Scores by Cell Type')
    # plt.show()
    
    # 概率求和>0.5
    # outliers_matrix=list(ct_specific_outliers.values())
    # outliers_matrix=[np.array(i) for i in outliers_matrix]
    # outliers_matrix=np.concatenate(outliers_matrix)

    # sorted_array = outliers_matrix[outliers_matrix[:, 0].argsort()[::-1]]

    # # Apply the softmax function
    # probabilities = softmax(sorted_array[:,0])
    # # Initialize cumulative sum and result list
    # cumulative_sum = 0
    # result_index = []

    # # Iterate over sorted probabilities and accumulate sum
    # for i,prob in enumerate(probabilities):
    #     cumulative_sum += prob
    #     result_index.append(int(sorted_array[i,1]))
